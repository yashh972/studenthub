...
/**
 * Sends a premium HTML email containing the password reset verification OTP code.
 * @param {string} toEmail - The recipient's email address
 * @param {string} otpCode - The 6-digit OTP code
 * @returns {Promise<{success: boolean, simulated: boolean}>}
 */
export async function sendPasswordResetEmail(toEmail, otpCode) {
  ...
  if (!smtpUser || !smtpPass) {
    console.log('\n==================================================');
    console.log(`📩 [FALLBACK SIMULATION] Sent Password Reset OTP to: ${toEmail}`);
    console.log(`🔑 Password Reset OTP Code: [ ${otpCode} ] (Expires in 10 mins)`);
    console.log('💡 Note: Configure SMTP_USER and SMTP_PASS in your .env file to enable real email sending.');
    console.log('==================================================\n');
    return { success: true, simulated: true };
  }
  ...
  try {
    ...
    const mailOptions = {
      ...
      text: `To reset your StudyHUB password, please use the 6-digit OTP code: ${otpCode}. It expires in 10 minutes.`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body>
          <div class="wrapper">
            <div class="container">
              ...
              <p class="text">
                We received a request to reset your StudyHUB account password. Please use the 6-digit verification code below to authorize this change. This code will expire in 10 minutes.
              </p>
              <div class="otp-container">
                <span class="otp-code">${otpCode}</span>
              </div>
              <p class="text">If you did not request a password reset, you can safely ignore this email and your password will remain unchanged.</p>
              <div class="footer">
                &copy; 2026 StudyHub. All rights reserved.<br>
                Empowering student collaboration and learning.
              </div>
    ...
    const info = await transporter.sendMail(mailOptions);
    console.log(`✉️ Password reset email successfully dispatched to ${toEmail}. MessageId: ${info.messageId}`);
    return { success: true, simulated: false };
  } catch (error) {
    console.error('❌ Error sending real password reset OTP email through SMTP:', error);
    console.log('\n==================================================');
    console.log(`📩 [ERROR FALLBACK SIMULATION] Sent Password Reset OTP to: ${toEmail}`);
    console.log(`🔑 Password Reset OTP Code: [ ${otpCode} ] (Expires in 10 mins)`);
    console.log('==================================================\n');
    return { success: false, simulated: true, error: error.message };
  }
